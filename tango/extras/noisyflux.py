...
class NoisyFluxSpaceTime(object):
    """Decorator that adds random noise correlated in space and time,
    generated by an AR(1) process, to the flux returned by an inherent
    ...
    def __init__(self, amplitude, tac_x, dx, tac_t, dt, fluxModel):
        ...
        self.fluxModel = fluxModel
        self.amplitude = amplitude   # scalar
        self.tac_x = tac_x / dx      # autocorrelation length measured in discrete units (does not have to be integer)
        self.phi_t = np.exp(- dt / tac_t)  # AR(1) parameter in time
        print("Phi = {}".format(self.phi_t))
        self._last_noise = {}        # Stores the noise from the last call, to provide correlation in time

    def get_flux(self, profiles):
        fluxes = self.fluxModel.get_flux(profiles)
        noisyFluxes = self._add_noise_to_fluxes(fluxes)
        return noisyFluxes

    def _get_last_noise(self, label, numSamples):
        """Returns the last noise for the given label.
        If this is the first call, then generate noise"""
        if label in self._last_noise:
            return self._last_noise[label]
        # No noise so generate
        return self.amplitude * ar1noise(numSamples, self.tac_x)

    def _add_noise_to_fluxes(self, fluxes):
        """Add noise to each field.  The noise added to each field is statistically independent."""
        noisyFluxes = {}
        for label in fluxes:
            flux = fluxes[label]
            ...
            noise = self.amplitude * ar1noise(numSamples, self.tac_x) # Correlated in space, uncorrelated in time
            last_noise = self._get_last_noise(label, numSamples)

            # AR(1) process in time
            # Note: Need to scale the noise so that the variance of the result is unchanged
            noise = self.phi_t * last_noise + noise * np.sqrt(1 - self.phi_t**2)

            self._last_noise[label] = noise.copy()

            # Damp noise close to both boundaries
            noise = dampen_sides(noise)

            noisyFluxes[label] = (1 + noise) * flux
        return noisyFluxes
